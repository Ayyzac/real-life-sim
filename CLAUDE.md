...
3. **Waktu berjalan sendiri saat bermain** (diubah atas keputusan user, 23 Sep 2026 — sebelumnya "waktu hanya maju kalau pemain menekan tombol"). 1 jam game = 1 menit nyata di kecepatan 1×, bisa di-pause dan dipercepat (2×/4×). **Satu-satunya timer ada di lapisan UI** (`src/ui/clock.ts`) dan hanya mengirim intent `tick` lewat store — simulation core tetap murni, tanpa `setInterval`/timer sendiri, dan bisa dites headless. Jam berhenti otomatis saat dialog, HP, laptop, mini-game terbuka, saat aksi berdurasi berjalan, dan saat tab browser tidak terlihat. Tidur dan "Lanjut Minggu" tetap ada.
